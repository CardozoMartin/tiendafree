            ...
            {orders.map((o: any) => (
              ...
              >
                ...
                <div className="mt-5 flex gap-2">
                    ...
                    >
                      {selectedOrder?.id === o.id ? 'Cerrar detalle' : 'Ver detalle'}
                    </button>
                    <a
                ...
                {/* DETALLE DESPLEGABLE */}
                {selectedOrder?.id === o.id && (
                  <div className="border-t border-slate-50 bg-slate-50/50 animate-expand-down">
                    <div className="p-6 space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[.9rem] font-bold text-slate-700">{o.compradorNombre}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <MI name="mail" className="text-slate-300 !text-lg" />
                                    <span className="text-[.9rem] text-slate-500">{o.compradorEmail}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <MI name="call" className="text-slate-300 !text-lg" />
                                    <span className="text-[.9rem] text-slate-500">{o.compradorTel}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm">
                              ...
                              <p className="text-[.85rem] font-bold text-slate-700 leading-tight">
                                  {o.direccionCalle} {o.direccionNumero}
                              </p>
                        ...
                        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                            ...
                            <div className="mt-6 pt-4 border-t border-dashed border-slate-100 flex flex-col gap-2">
                                <div className="flex justify-between items-center text-[.85rem] text-slate-500 font-medium">
                                    <span>Subtotal</span>
                                    <span>${(Number(o.total) - Number(o.costoEnvio || 0)).toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-center text-[.85rem] text-slate-500 font-medium">
                                    <span>Envío</span>
                                    <span>${Number(o.costoEnvio || 0).toLocaleString()}</span>
                                </div>
                        ...
                        <div className="pt-2 grid grid-cols-2 gap-3">
                            {o.estado === 'PENDIENTE' && (
                                <button 
                                  onClick={(e) => { e.stopPropagation(); updateStatus({ id: o.id, estado: 'CONFIRMADO' }); }}
                            ...
                            {o.estado === 'EN_CAMINO' && (
                                 <button 
                                  onClick={(e) => { e.stopPropagation(); updateStatus({ id: o.id, estado: 'ENTREGADO' }); }}
                            ...
                            {o.estado !== 'ENTREGADO' && o.estado !== 'CANCELADO' && (
                                <button 
                                 onClick={(e) => { e.stopPropagation(); updateStatus({ id: o.id, estado: 'CANCELADO' }); }}
