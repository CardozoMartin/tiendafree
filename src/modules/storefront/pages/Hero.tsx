...
import { ArrowRight, ShoppingBag, Sparkles, Star, Store, MousePointer2, Palette, Package, Inbox, Check, X, Globe } from 'lucide-react';
...
function DemoWindow() {
  ...
  const changeDashView = (v: any) => { setDashView(v); dashViewRef.current = v; };
       ...
       while(!unmounted) {
          ...
          if (titleInputRef.current) {
              await animateCursorTo(titleInputRef.current, () => {
                  gsap.fromTo(titleInputRef.current, { scale: 1 }, { scale: 0.98, duration: 0.1, yoyo: true, repeat: 1 });
              });
              await delay(400);
          ...
          await animateCursorTo(colorRefs.current[nextTheme], () => {
             ...
             if (colorRefs.current[nextTheme]) {
                 gsap.fromTo(colorRefs.current[nextTheme], { scale: 1 }, { scale: 0.7, duration: 0.1, yoyo: true, repeat: 1 });
             }
          });
          if (unmounted) break;
          await delay(1200);
          ...
          await animateCursorTo(tabStoreWebRef.current, () => {
              if (tabStoreWebRef.current) gsap.fromTo(tabStoreWebRef.current, { scale: 1 }, { scale: 0.9, duration: 0.1, yoyo: true, repeat: 1 });
          });
          if (unmounted) break;
          ...
          await animateCursorTo(addToCartRef.current, () => {
             setCartCount(c => c + 1);
             if (cartIconRef.current) gsap.fromTo(cartIconRef.current, { scale: 1 }, { scale: 1.25, duration: 0.15, yoyo: true, repeat: 1, ease: 'power2.inOut' });
             if (addToCartRef.current) gsap.fromTo(addToCartRef.current, { scale: 1 }, { scale: 0.95, duration: 0.1, yoyo: true, repeat: 1 });
          });
          if (unmounted) break;
          await delay(1000);
          ...
          if (acceptOrderRef.current) {
             await animateCursorTo(acceptOrderRef.current, () => {
                ...
                gsap.fromTo(acceptOrderRef.current, { scale: 1 }, { scale: 0.9, duration: 0.1, yoyo: true, repeat: 1 });
             });
          }
          if (unmounted) break;
          await delay(1200);
  ...
  return (
    ...
    >
      ...
      <div
         ...
         style={{ opacity: activeApp === 'dashboard' ? 1 : 0, pointerEvents: activeApp === 'dashboard' ? 'auto' : 'none' }}>
         ...
         <div className="hidden md:flex flex-col shrink-0 bg-white border-r border-zinc-100 relative w-[228px]">
            ...
            <div className="flex items-center h-14 border-b border-zinc-100 px-3.5 shrink-0 gap-2.5">
               <div className="size-7 rounded-lg flex items-center justify-center text-white shrink-0" style={{ backgroundColor: theme.hex, transition: 'all 0.3s' }}>
            ...
            <nav className="flex-1 p-2 space-y-0.5 mt-2">
               ...
               <button
                  ...
                  style={dashView === 'editor' ? { backgroundColor: `${theme.hex}15`, color: theme.hex } : { color: '#71717a' }}
               >
                  <Palette size={17} />
               ...
               <button
                  ...
                  style={dashView === 'products' ? { backgroundColor: `${theme.hex}15`, color: theme.hex } : { color: '#71717a', backgroundColor: 'transparent' }}
               >
                  <Package size={17} />
               ...
               <button
                  ...
                  style={dashView === 'orders' ? { backgroundColor: `${theme.hex}15`, color: theme.hex } : { color: '#71717a', backgroundColor: 'transparent' }}
               >
                  <Inbox size={17} />
            ...
            <div className="shrink-0 border-t border-zinc-100 flex items-center gap-2.5 px-3 py-3">
               <div className="w-[30px] h-[30px] rounded-full flex items-center justify-center font-bold text-[11px]" style={{ backgroundColor: `${theme.hex}18`, color: theme.hex, transition: 'all 0.3s' }}>
                  MC
               </div>
               <div className="flex-1 min-w-0">
                  <p className="text-[12.5px] font-bold text-zinc-900 truncate leading-tight">M. Cardozo</p>
                  <p className="text-[11px] text-zinc-400 truncate leading-tight mt-0.5">user@vitrina.app</p>
               </div>
            </div>
         </div>
               ...
               <div className="flex flex-col h-full">
                  ...
                  <div className="p-4 flex-1 flex flex-col gap-4 overflow-y-auto">
                     <div className="bg-white rounded-xl border border-zinc-100 p-4 shadow-sm">
                        <label className="block text-xs font-bold text-zinc-700 mb-2 uppercase tracking-wide">Nombre</label>
                        <div
                           ref={titleInputRef}
                     ...
                     <div className="bg-white rounded-xl border border-zinc-100 p-4 shadow-sm">
                        <label className="block text-xs font-bold text-zinc-700 mb-3 uppercase tracking-wide">Color principal</label>
                           ...
                           {storeThemes.map((t, i) => (
                               <div
                                  ...
                                  style={{
                                     ...
                                     boxShadow: i === themeIdx ? '0 0 0 2px #fff inset' : '0 1px 2px rgba(0,0,0,0.1)'
                                  }}
                               />
                           ))}
                        </div>
                 ...
                 <div className="p-4 flex-1 flex flex-col gap-4 overflow-y-auto">
                    <div className="bg-white rounded-xl border border-zinc-100 p-4 shadow-sm">
                       <div className="flex justify-between items-center mb-3">
                          ...
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${orderStatus === 'accepted' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                             {orderStatus === 'accepted' ? 'PREPARANDO' : 'PENDIENTE'}
                          </span>
                       </div>
                       <div>
                          <p className="text-[13px] font-bold text-zinc-800 leading-tight">Camila Fernandez</p>
                          <p className="text-[11px] text-zinc-500 mt-1 mb-4">MercadoPago • Envío Rápido</p>
                       </div>
                       {orderStatus === 'pending' ? (
                           <button ref={acceptOrderRef} className="w-full flex items-center justify-center gap-2 py-2.5 bg-zinc-900 text-white text-[12px] font-bold rounded-lg transition-all">
            ...
            {dashView === 'products' && (
               <div className="flex flex-col h-full">
                 <div className="px-5 py-4 border-b border-zinc-100 bg-white shrink-0">
                    ...
                    <p className="text-[11px] text-zinc-500 mt-1.5">{demoProducts.length} productos activos</p>
                 </div>
                 <div className="p-4 flex-1 flex flex-col gap-2.5 overflow-y-auto">
                    {demoProducts.slice(0, 3).map((p, i) => (
                       <div key={i} className="bg-white rounded-xl border border-zinc-100 p-2.5 flex items-center gap-3 shadow-sm">
                          <img src={p.img} className="w-9 h-9 rounded-md object-cover ring-1 ring-zinc-100" />
                          <div className="flex-1 min-w-0">
                             <p className="text-[12px] font-bold text-zinc-900 truncate leading-tight">{p.name}</p>
      ...
      </div> {/* Closes DASHBOARD VIRTUAL OS LAYER */}
      ...
      <div
         ...
         style={{ opacity: activeApp === 'store' ? 1 : 0, pointerEvents: activeApp === 'store' ? 'auto' : 'none' }}>
         <div className="flex-1 bg-[#181311] rounded-t-xl overflow-hidden flex flex-col shadow-[0_10px_30px_rgba(0,0,0,0.15)] ring-1 ring-black/5">
            ...
            >
              ...
              >
                {storeTitle.replace('|', '').toLowerCase().replace(/ /g, '') || 'mitienda'}.tiendafree.com
              </div>
            </div>

            {/* Contenido de la tienda demo */}
            <div style={{ padding: '0', background: '#f7f4ef', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span
              style={{ fontSize: 13, fontWeight: 800, color: '#17120f', letterSpacing: '-0.03em' }}
            >
              {storeTitle.replace('|', '') || '\u00A0'}
            </span>
          </div>
          <div
            style={{ display: 'flex', gap: 12, fontSize: 11, color: '#7d5d48', fontWeight: 600 }}
          >
            <span>Inicio</span>
        ...
        <div style={{ padding: '1rem 1.25rem', display: 'flex', gap: '0.75rem', flex: 1, minHeight: 0 }}>
          {/* Producto principal */}
            ...
            <div style={{ marginTop: '0.6rem', padding: '0 2px' }}>
              ...
              >
                ...
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 16, fontWeight: 900, color: theme.hex, margin: 0, transition: 'color 0.3s ease' }}>
                    {product.price}
                  </p>
                  {product.oldPrice && (
                    <p
            ...
            {demoProducts.map((p, i) => (
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 10, fontWeight: 800, color: theme.hex, margin: '1px 0 0', transition: 'color 0.3s ease' }}>
                    {p.price}
                  </p>
                </div>
              </button>
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      >
        <Link to="/" className="flex items-center gap-3">
          <div className="flex size-10 items-center justify-center rounded-xl bg-[#181311] text-white">
            <img src={ICON} alt="TiendiZi" className="w-100 h-20" />
          </div>
          <div>
      ...
      {/* ── Copy central ───────────────────────────────────────────────────── */}
      <div className="mx-auto max-w-5xl px-6 pt-8 pb-10 text-center sm:px-8 lg:px-10">
        {/* Badge */}
        <div
          ref={badgeRef}
          className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-[11px] font-bold uppercase tracking-[0.22em]"
          style={{
            background: 'rgba(203,183,255,0.25)',
            border: '1px solid rgba(124,107,255,0.2)',
            color: '#5a4acc',
          }}
        >
          <Sparkles className="h-3.5 w-3.5" />
          Gratis para siempre · Sin comisiones
        </div>
        ...
        >
          Creá tu tienda en minutos, recibí pedidos por WhatsApp y vendé sin pagar comisiones.
        ...
        >
          <Link
            to="/register"
            className="inline-flex h-14 items-center justify-center gap-2 rounded-full bg-[#181311] px-9 text-sm font-bold uppercase tracking-[0.14em] text-white hover:bg-[#2c241f] transition-colors shadow-[0_8px_24px_rgba(23,18,15,0.18)]"
          >
      ...
      >
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse at center, rgba(124, 107, 255, 0.18) 0%, rgba(255, 107, 61, 0.08) 40%, rgba(255, 255, 255, 0) 70%)',
            filter: 'blur(80px)'
          }}
        />
