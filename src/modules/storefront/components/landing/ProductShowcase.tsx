...
const FEATURE_DATA = [
  {
    id: 1,
    title: 'Gestion 1-Tap',
    desc: 'Gestiona tu stock en tiempo real desde tu movil. Cero friccion.',
  },
  {
    id: 2,
    title: 'Pedidos en WhatsApp',
    desc: 'Convierte chats en ventas. Recibe los pedidos ordenados directamente en tu WhatsApp.',
  },
  {
    id: 3,
    title: '0% Comisiones',
    desc: 'Lo que vendes es tuyo. No retenemos tu dinero ni cobramos por venta.',
  },
];
...
function FeatureItem({
  ...
  feature: (typeof FEATURE_DATA)[number];
  index: number;
  ...
  return (
    <motion.div style={{ opacity }} className="flex flex-col gap-4">
      <span className="text-tiendzi-accent font-bold tracking-widest text-sm uppercase">
        0{feature.id}
      </span>
...
function MockupPanel({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  const rotateX = useTransform(scrollYProgress, [0, 1], [5, -5]);
  const rotateY = useTransform(scrollYProgress, [0, 1], [-5, 5]);
  const y = useTransform(scrollYProgress, [0, 0.4, 0.8], ['0%', '-100%', '-200%']);
    ...
    <motion.div className="w-full h-full relative" style={{ perspective: 1000 }}>
      ...
      >
        ...
        <div className="relative flex-1 overflow-hidden p-6">
          <motion.div className="absolute inset-0 p-6 flex flex-col gap-4" style={{ y }}>
      ...
      <div className="w-full max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 relative">
        ...
        <div className="hidden lg:block relative">
          <div className="sticky top-1/2 -translate-y-1/2 h-[600px] w-full bg-tiendzi-card rounded-[2rem] border border-white/5 overflow-hidden flex items-center justify-center shadow-2xl p-8">
            <MockupPanel scrollYProgress={scrollYProgress} />
          </div>
        </div>
      </div>
    </section>
